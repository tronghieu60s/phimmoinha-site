import { GENRE_LIST, PAGE } from '@const/path';
...
export default function GenresSlugPage(props: Props) {
  const { options } = props;
  ...
  const breadcrumb = useMemo(
    (): BreadcrumbType[] => [
      {
        label: t('film-list:breadcrumb.genre'),
        path: `/${GENRE_LIST}`,
      },
      {
        label: `${term?.term_name}`,
        path: `/${GENRE_LIST}/${term?.term_slug}`,
      },
    ],
    [t, term?.term_name, term?.term_slug],
  );

  const keywordText = options?.keywordsConfig?.data?.option_value?.genre;
...
export const getStaticPaths: GetStaticPaths = async () => {
  const termsItem = await getTerms({
    params: {
      search: { term_taxonomy: 'movie_category' },
      loadAll: true,
    },
  });
