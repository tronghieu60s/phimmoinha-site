import { PAGE, PUBLISH_LIST } from '@const/path';
import { PAGE_TEXT, PUBLISH_TEXT } from '@const/text';
...
export default function PublishSlugPage(props: Props) {
  const { options } = props;
...
export const getStaticPaths: GetStaticPaths = async () => {
  const paths = PUBLISH_DATA?.map((item) => ({
    params: { slug: String(item), page: `${PAGE}-1` },
  }));
  ...
  return {
    props: {
      ...(await getCommonData(context, {
        title: `${PUBLISH_TEXT} ${slug} - ${PAGE_TEXT} ${page}`,
        sidebar: true,
      })),
      slug,
      movies,
