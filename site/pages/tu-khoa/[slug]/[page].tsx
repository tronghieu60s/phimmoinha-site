import { PAGE, TAG_LIST } from '@const/path';
import { PAGE_TEXT, TAG_TEXT } from '@const/text';
...
export default function TagsSlugPage(props: Props) {
  const { options } = props;
  ...
  const breadcrumb = useMemo(
    (): BreadcrumbType[] => [
      {
        label: t('film-list:breadcrumb.tag'),
        path: `/${TAG_LIST}`,
      },
      {
        label: `${term?.term_name}`,
        path: `/${TAG_LIST}/${term?.term_slug}`,
      },
    ],
  ...
  return (
    <FilmContainer
      headingText={`${t('film-list:heading.tag')}: ${term?.term_name}`}
      keywordText={t(
        '',
...
export const getStaticPaths: GetStaticPaths = async () => {
  const termsItem = await getTerms({
    params: {
      search: { term_taxonomy: 'movie_tag' },
    },
  });
  ...
  return {
    props: {
      ...(await getCommonData(context, {
        title: `${TAG_TEXT}: ${term?.term_name} - ${PAGE_TEXT} ${page}`,
        sidebar: true,
      })),
      term,
      movies,
