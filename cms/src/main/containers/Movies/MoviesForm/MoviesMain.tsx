...
export default function MoviesMain(props: Props) {
  ...
  return (
    <Row justify="start" className="px-2" gutter={40}>
      <Col span={24}>
        <FormItem
          name="Title"
          label={t('movies:form.input.title')}
          rules={[{ required: true }]}
          {...(errorTitle && {
            hasFeedback: true,
            help: errorTitle,
      ...
      <Col span={24}>
        <FormItem
          name="Slug"
          label={t('movies:form.input.slug')}
          {...(errorSlug && {
            hasFeedback: true,
            help: errorSlug,
            validateStatus: 'warning',
      ...
      <Col span={24}>
        <FormItem name="Content" label={t('movies:form.input.content')}>
          <FormInputEditor className="h-editor-600" onChange={onChangeContent} />
        </FormItem>
        <Row justify="space-between">
          <pre>{t('movies:form.input.content.count', { count: countContent })}</pre>
        </Row>
      </Col>
    </Row>
  );
